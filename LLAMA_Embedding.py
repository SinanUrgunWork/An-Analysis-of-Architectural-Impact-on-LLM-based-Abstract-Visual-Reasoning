...
REFERENCE_EMBEDDINGS_FILE = "reference_solution_embeddings_groq.json"
...
class ReferenceSolutionManager:
    ...
    def initialize_default_references(self):
        default_patterns = [
            ...
            {
                "reasoning": "EXAMINE STRUCTURE: All panels show consistent object arrangements. IDENTIFY PATTERNS: Clear progression in shape properties. DETECT TRANSFORMATIONS: Size increases systematically. APPLY RULE: Continue transformation pattern. GENERATE SOLUTION: Create matching panel.",
                "function_call": {"config_type": "left_right", "objects": [{"shape": "hexagon", "size": 0.7, "color": 4, "angle": 90}]},
                "metadata": {"type": "step_by_step_analysis", "confidence": 9.2}
            },
            {
                "reasoning": "Row 1 analysis: Objects maintain positions but colors change. Row 2 analysis: Similar color pattern continues. Pattern rule: Colors follow sequence 2-5-8 across rows. Column verification: Positions remain stable. Solution: Apply color rule to final position.",
                "function_call": {"config_type": "up_down", "objects": [{"shape": "pentagon", "size": 0.5, "color": 8, "angle": 0}]},
                "metadata": {"type": "detailed_row_column", "confidence": 8.9}
            }
        ]
...
def process_file_with_embedding(result, max_trials=3):
    try:
        ...
        if not result["success"]:
            ...
            cot_filename = f"embedding_cot_reasoning_question{file_number}.json"
            cot_data = {
                "puzzle_file": filepath,
                "model_used": MODEL_NAME,
                "processing_type": "direct_processing_with_embedding",
                "solving_session": {
        ...
        print(f"[Question {file_number}] Initial CoT length: {len(initial_reasoning)} chars")
        ...
        for attempt in range(max_trials):
            print(f"[Question {file_number}] Embedding validation attempt {attempt + 1}")
            
            validation_result = perform_embedding_validation(current_reasoning, current_function_call, file_number)
            ...
            print(f"[Question {file_number}] Attempt {attempt + 1}: Similarity={similarity_score:.3f} | Confidence={confidence_score:.1f} | Recommendation={recommendation}")
            ...
            if similarity_score >= SIMILARITY_THRESHOLD and recommendation == "ACCEPT":
                ...
                print(f"[Question {file_number}] Generating new attempt...")
                new_reasoning, new_function_call = generate_new_attempt(file_number, attempt + 1)
                
                if new_reasoning and new_function_call:
                    current_reasoning = new_reasoning
                    current_function_call = new_function_call
                    print(f"[Question {file_number}] New attempt generated, length: {len(new_reasoning)} chars")
                else:
                    print(f"[Question {file_number}] Failed to generate new attempt")
                    final_similarity = similarity_score
                    final_confidence = confidence_score
        ...
        cot_data = {
            ...
            "reasoning_analysis": {
                "has_step1_patterns": any(keyword in current_reasoning.lower() for keyword in ["examine", "structure", "step 1"]),
                "has_step2_rule": any(keyword in current_reasoning.lower() for keyword in ["identify", "pattern", "step 2"]),
                "has_step3_apply": any(keyword in current_reasoning.lower() for keyword in ["detect", "transformation", "step 3"]),
                "has_step4_generate": any(keyword in current_reasoning.lower() for keyword in ["apply", "rule", "step 4"]),
                "has_step5_solution": any(keyword in current_reasoning.lower() for keyword in ["generate", "solution", "step 5"]),
                "mentions_rows_columns": "row" in current_reasoning.lower() and "column" in current_reasoning.lower(),
                "mentions_shapes": any(shape in current_reasoning.lower() for shape in ["triangle", "square", "pentagon", "hexagon", "circle"]),
                "mentions_colors": any(color in current_reasoning.lower() for color in ["black", "gray", "white", "color"]),
                "mentions_positions": "position" in current_reasoning.lower() or "grid" in current_reasoning.lower(),
                "cot_captured_successfully": len(current_reasoning) > 50,
                "direct_processing": True
            },
        ...
        if current_function_call is not None:
            current_function_call["source_filename"] = filepath
        execute_result = execute_tool_function("generate_visual_panel", current_function_call, filepath)
...
def main():
    print("=== Groq Llama-3.3-70b + OpenAI Embeddings RAVEN Matrix Solver ===")
    ...
    if not os.getenv("LLAMA_API_KEY"):
        print("ERROR: Missing LLAMA_API_KEY in .env file")
        return
    ...
    for i, json_file_path in enumerate(json_files, 1):
        ...
        final_result = process_file_with_embedding(file_result)
    ...
    summary_filename = f"groq_embedding_processing_summary_{int(time.time())}.json"
