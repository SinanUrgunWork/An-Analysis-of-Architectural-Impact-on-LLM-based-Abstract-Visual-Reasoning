...
def perform_self_reflection(reasoning, function_call, question_number):
    ...
    json_file_path = os.path.join(INPUTS_FOLDER, f"geminiAnswer{question_number}.json")
...
def process_batch_result_with_reflection(result, question_number, max_trials=3):
    try:
        ...
        cot_data = {
            "puzzle_file": f"inputs/geminiAnswer{question_number}.json",
...
def main():
    print("=== CHUNKED Batch API RAVEN Matrix Solver with Self-Reflection ===")
    ...
    if not json_files:
        print("No geminiAnswer*.json files found in inputs folder!")
        return
