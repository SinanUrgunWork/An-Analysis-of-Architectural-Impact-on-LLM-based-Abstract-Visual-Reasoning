...
"""
        ...
        response = client.messages.create(
            ...
            max_tokens=4000,
            temperature=0.1,
            system="You are a configuration pattern specialist. Focus ONLY on spatial arrangements and layout patterns. Ignore shapes, colors, sizes, and rotations.",
            messages=[
...
"""
        ...
        response = client.messages.create(
            ...
            max_tokens=4000,
            temperature=0.1,
            system="You are a shape pattern specialist. Focus ONLY on shape types and shape transformations. Ignore configurations, colors, sizes, and rotations.",
            messages=[
                {"role": "user", "content": prompt}
            ]
        )
...
"""
        ...
        response = client.messages.create(
            ...
            max_tokens=4000,
            temperature=0.1,
            system="You are a color pattern specialist. Focus ONLY on color values and color progressions. Ignore configurations, shapes, sizes, and rotations.",
            messages=[
                {"role": "user", "content": prompt}
            ]
        )
...
"""
        ...
        response = client.messages.create(
            ...
            max_tokens=4000,
            temperature=0.1,
            system="You are a size pattern specialist. Focus ONLY on size values and size scaling patterns. Ignore configurations, shapes, colors, and rotations.",
            messages=[
                {"role": "user", "content": prompt}
            ]
        )
...
"""
        ...
        response = client.messages.create(
            ...
            max_tokens=4000,
            temperature=0.1,
            system="You are a rotation pattern specialist. Focus ONLY on rotation values and rotational transformations. Ignore configurations, shapes, colors, and sizes.",
            messages=[
                {"role": "user", "content": prompt}
            ]
        )
...
Use the exact parameter values and terminology from the specialist analyses."""
        ...
        for content_block in response.content:
            if content_block.type == "text":
                cot_reasoning += content_block.text
...
def process_all_json_files():
    ...
    if successful_outputs:
        safe_print(f"\nGenerated files:")
        for i, output in enumerate(successful_outputs[:10]):
            safe_print(f"  - {output}")
...
def main():
    ...
    safe_print("- Master Agent: Synthesis Coordinator (never sees raw data)")
    
    if not os.getenv("ANTHROPIC_API_KEY"):
        safe_print("[!] Missing ANTHROPIC_API_KEY in .env file")
        return
