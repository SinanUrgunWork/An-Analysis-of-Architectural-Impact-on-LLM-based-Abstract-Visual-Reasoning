...
class RavenAgentSystem:
    ...
    def analyze_config_types(self):
        """SLAVE AGENT 1: Analyze configuration patterns ONLY """
...
"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are a configuration pattern specialist. Focus ONLY on spatial arrangements and layout patterns. Ignore shapes, colors, sizes, and rotations."},
...
"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are a shape pattern specialist. Focus ONLY on shape types and shape transformations. Ignore configurations, colors, sizes, and rotations."},
                {"role": "user", "content": prompt}
            ]
        )
...
"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are a color pattern specialist. Focus ONLY on color values and color progressions. Ignore configurations, shapes, sizes, and rotations."},
                {"role": "user", "content": prompt}
            ]
        )
...
"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are a size pattern specialist. Focus ONLY on size values and size scaling patterns. Ignore configurations, shapes, colors, and rotations."},
                {"role": "user", "content": prompt}
            ]
        )
...
AVAILABLE ANGLES:
- Rotation in degrees (STRICTLY -180 to 180, integers only)
- Common values: -180, -135, -90, -45, 0, 45, 90, 135, 180
- CRITICAL: Never recommend angles outside -180 to 180 range (e.g., 225° should be -135°, 270° should be -90°)
...
5. Determine what angles panel 3_3 should have (ENSURE all angles are -180 to 180)
...
Provide your analysis and conclude with: "RECOMMENDED ANGLES: [list of angles between -180 and 180]"
"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are a rotation pattern specialist. Focus ONLY on rotation values and rotational transformations. Ignore configurations, shapes, colors, and sizes."},
                {"role": "user", "content": prompt}
            ]
        )
    ...
    def save_agent_analyses(self):
        ...
        agent_data = {
            ...
            "processing_type": "true_multi_agent_specialist_analysis",
    ...
    def master_solver(self):
        ...
        # MASTER PROMPT - NO RAW INPUT DATA, ONLY SLAVE ANALYSES
...
- Angles MUST be integers between -180 and 180 (inclusive)
- If any angle is outside this range, normalize it (e.g., 225° → -135°, 270° → -90°, 360° → 0°)
- Sizes MUST be floats between 0.1 and 1.0
- Colors MUST be integers 0-9 or valid string names

Follow the 5 steps above, then call the function with the synthesized specialist recommendations."""

        # SAME SYSTEM PROMPT as app3.py
...
Use the exact parameter values and terminology from the specialist analyses."""
        ...
        # Capture CoT reasoning from the response - with null check
        ...
        if message.tool_calls:
            for call in message.tool_calls:
                ...
                master_cot_data = {
                    ...
                    "timestamp": self.timestamp,
                    "model_used": MODEL_NAME,
                    "processing_type": "true_multi_agent_cot",
                    "solving_session": {
                        ...
                        "success": True,
                        "extraction_method": "multi_agent_synthesis"
                    },
                    "true_multi_agent_system": {
                ...
                with open(llm_output_filename, "w", encoding="utf-8") as out:
                    json.dump({
                        "worker_id": self.worker_id,
                        "function_arguments": arguments,
                        "note": f"True multi-agent master synthesis CoT reasoning details are in {master_cot_filename}"
            ...
            failed_cot_data = {
                ...
                "solving_session": {
                    ...
                    "extraction_method": "multi_agent_synthesis_failed"
                },
                "true_multi_agent_system": {
                    "master_sees_raw_data": False,
                    "master_only_sees_agent_analyses": True,
...
def get_json_files_from_inputs():
    ...
    for filename in os.listdir(INPUTS_FOLDER):
        if filename.startswith('geminiAnswer') and filename.endswith('.json'):
    ...
    def extract_number(filepath):
        ...
        match = re.search(r'geminiAnswer(\d+)', filename)
...
def solve_raven_from_json(json_file_path):
    """
    True Multi-agent RAVEN solver: Slaves analyze raw data, Master synthesizes slave outputs
    """
    try:
        if not os.path.exists(json_file_path):
            return {"status": "error", "message": f"JSON file not found: {json_file_path}"}
        
        safe_print(f"[{threading.current_thread().name}] Starting True Multi-Agent Raven Solver...")
        ...
        if number_match:
            question_number = number_match.group()
            error_filename = f"true_multi_agent_error_question{question_number}.json"
        else:
            error_filename = f"true_multi_agent_error_{base_name}.json"
...
def process_all_json_files():
    ...
    if not json_files:
        safe_print("No geminiAnswer*.json files found in inputs folder!")
        return
    ...
    summary_data = {
        "processing_session": {
            ...
            "failed": failed,
            "model_used": MODEL_NAME,
            "processing_type": "true_multi_agent_cot_parallel",
    ...
    with open("true_multi_agent_batch_summary_parallel.json", "w", encoding="utf-8") as f:
    ...
    safe_print("Summary saved to: true_multi_agent_batch_summary_parallel.json")
...
def main():
    """True Multi-agent automated RAVEN solver with specialist coordination"""
    ...
    if not os.getenv("GROQ_API_KEY"):
        safe_print("[!] Missing GROQ_API_KEY in .env file")
        return
